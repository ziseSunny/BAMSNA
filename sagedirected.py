...
class SAGEDirected(MessagePassing):
    def __init__(self, K=1,  cached=False, bias=True,
                 **kwargs):
        super(SAGEDirected, self).__init__(aggr='mean', **kwargs)
        self.K = K
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        xs_r = F.normalize(self.propagate(edge_index, x=x[:,:int(x.size(1)/2)], norm=norm_r), p=2, dim=1)
            # xs_r.append(self.propagate(edge_index, x=xs_r[-1], norm=norm_r))
        xs_t = F.normalize(self.propagate(edge_index, x=x[:,int(x.size(1)/2):], norm=norm_t), p=2, dim=1)
            # xs_t.append(self.propagate(edge_index, x=xs_t[-1], norm=norm_t))
        xs_r = torch.cat([x[:,:int(x.size(1)/2)], xs_r], dim=1)
        xs_t = torch.cat([x[:,int(x.size(1)/2):], xs_t], dim=1)
